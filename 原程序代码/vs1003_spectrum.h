...
/*对应寄存器的数据*/
unsigned short code dtab[943] = { /* Data to write */
...
/************************************************************************/
//函数名:vs1003_load_patch
//功  能:为VS1003打补丁程序　使其具有频谱功能
//备  注:必须在软复位之后使用才有效
/************************************************************************/
void vs1003_load_patch()
{
	unsigned short i;
	for(i=0;i<943;i++)
		vs1003_sci_write(atab[i],dtab[i]);
}
...
//函数名:vs1003_get_spec
//功  能:从VS1003中读取频谱数据 
//参  数:spec为存放频谱数据的缓冲区 spec元素个数必须不小于14
//备  注:VS1003一次可输出14个频谱值
/************************************************************************/
void vs1003_get_spec(unsigned short *spec)
{
	unsigned char i;
	vs1003_sci_write(VS1003_SCI_WRAMADDR,0x1804);//0x1804写入WRAMADDR起动读取频谱
	for(i=0;i<14;i++)
		spec[i]=vs1003_sci_read(VS1003_SCI_WRAM);//从WRAM中读取
}
